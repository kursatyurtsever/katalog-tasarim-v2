...
export function ColorOpacityPicker({ color, opacity, onChange }: Props) {
  ...
  // Tıklanan yere göre popover'ın koordinatlarını hesaplıyoruz
  ...
  useEffect(() => {
    if (isOpen && buttonRef.current) {
      ...
      // Butonun sağ alt tarafına hizalayalım (Panelden taşmayı önlemek için sağa doğru açılacak)
      setCoords({
        top: rect.bottom + window.scrollY + 5,
        left: rect.right + window.scrollX - 210, // 210px popover genişliği, sağa hizalı
      });
    }
  }, [isOpen]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      // Hem butona hem de portaldaki popover'a tıklanmadığını kontrol ediyoruz
      if (
        isOpen && 
        buttonRef.current && 
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute inset-0 transition-colors" style={{ backgroundColor: color, opacity: opacity / 100 }} />
      </div>

      {/* PORTAL KULLANIMI: Popover artık Sidebar'ın içinde değil, doğrudan body'nin içinde olacak */}
      {isOpen && createPortal(
        <div 
          ref={popoverRef}
          ...
          style={{ top: `${coords.top}px`, left: `${coords.left}px` }} // Hesaplanan koordinatlara yerleştiriyoruz
        >
