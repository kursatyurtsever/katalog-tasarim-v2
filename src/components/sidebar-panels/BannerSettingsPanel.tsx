...
export function BannerSettingsPanel({ isOpen, onToggle }: Props) {
  ...
  return (
    <div className="bg-white rounded-md border border-fuchsia-200 shadow-sm mb-4 relative z-30">
      <button onClick={onToggle} className={`w-full flex items-center justify-between p-3 bg-fuchsia-600 hover:bg-fuchsia-500 transition-colors ${isOpen ? "rounded-t-md" : "rounded-md"}`}>
      ...
      {isOpen && (
        <div className="p-3 bg-fuchsia-50 border-t border-fuchsia-200 space-y-4 rounded-b-md">
          
          <div className="bg-white p-2 rounded border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[9px] font-bold text-slate-500 uppercase">Banner Izgarası (8x4)</span>
              <span className="text-[8px] text-slate-400">Çoklu seçim: Ctrl veya Shift</span>
            </div>
            
            <div className="grid grid-cols-8 gap-[1px] bg-slate-300 border border-slate-300 p-[1px] rounded-sm">
            ...
            {selectedBannerCellIds.length === 0 ? (
              ...
              <div className="bg-white p-3 rounded border border-blue-200 shadow-sm space-y-4">
                ...
                {selectedBannerCellIds.length === 1 && (
                  <>
                    <div className="space-y-1 border-t border-slate-100 pt-3 relative z-[70]">
                      <span className="text-[9px] font-bold text-slate-500">Hücre Metni</span>
                      <textarea rows={2} value={refCell!.text} onChange={(e) => updateBannerCell(refCell!.id, { text: e.target.value })} className="w-full text-[10px] font-bold text-slate-700 border border-slate-200 rounded p-1.5 outline-none focus:border-blue-500 resize-none" placeholder="Hücreye yazılacak metin..." />
                    </div>
                    <div className="space-y-1 border-t border-slate-100 pt-3 relative z-[60]">
                      ...
                      <div className="flex gap-1">
                        <label htmlFor="banner-image-upload" className="flex-1 h-[30px] flex items-center justify-center text-[10px] font-bold text-slate-600 bg-slate-50 border border-slate-200 hover:border-blue-400 rounded cursor-pointer transition-all">
                ...
                <div className="flex items-center justify-between border-t border-slate-100 pt-3 relative z-[50]">
                  <span className="text-[10px] font-bold text-slate-600">Hücre Dolgu Rengi</span>
                  <ColorOpacityPicker color={refCell!.bgColor.c} opacity={refCell!.bgColor.o} onChange={(c, o) => updateSelectedBannerCells({ bgColor: { c, o } })} />
                </div>

                <div className="border-t border-slate-100 pt-3 space-y-2 relative z-[40]">
                  <span className="text-[10px] font-bold text-slate-600">Hücre Çerçevesi (Border)</span>
                  <div className="flex items-center justify-between">
                    ...
                    <select value={refCell!.border.style} onChange={(e) => updateSelectedBannerCells({ border: { style: e.target.value as any } })} className="text-[10px] text-slate-600 border border-slate-200 rounded p-1 outline-none">
                  ...
                  <div className="grid grid-cols-5 gap-1 bg-slate-50 p-1.5 rounded border border-slate-100">
                ...
                <div className="border-t border-slate-100 pt-3 relative z-[5]">
                  <button 
                    onClick={() => {
