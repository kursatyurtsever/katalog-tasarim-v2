...
export function BannerSettingsPanel({ isOpen, onToggle }: Props) {
  const { 
  ...
  return (
    <div className="bg-slate-800 rounded-md border border-slate-700 shadow-lg mb-4 relative z-30">
      <button onClick={onToggle} className={`w-full flex items-center justify-between p-3 bg-slate-800 hover:bg-slate-700 transition-colors ${isOpen ? "rounded-t-md" : "rounded-md"}`}>
        <span className="text-[11px] font-black text-white uppercase tracking-widest">Banner Ayarları</span>
        <span className="text-white text-xs">{isOpen ? "▲" : "▼"}</span>
      </button>
      
      {isOpen && (
        <div className="p-3 bg-slate-800 border-t border-slate-700 space-y-4 rounded-b-md">
          
          <div className="bg-slate-900 p-2 rounded border border-slate-700 shadow-sm">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[9px] font-bold text-slate-300 uppercase">Banner Izgarası (8x4)</span>
              <span className="text-[8px] text-slate-400">Çoklu seçim: Ctrl veya Shift</span>
            </div>
            
            <div className="grid grid-cols-8 gap-[1px] bg-slate-700 border border-slate-700 p-[1px] rounded-sm">
              {bannerSettings.cells.filter(c => !c.hidden).map(cell => (
                <div
                  ...
                  className={`min-h-[24px] flex items-center justify-center text-[8px] cursor-pointer transition-colors overflow-hidden ${
          ...
          <div className="space-y-3">
            {selectedBannerCellIds.length === 0 ? (
              ...
              <div className="bg-slate-900 p-3 rounded border border-slate-700 shadow-sm space-y-4">
                ...
                {selectedBannerCellIds.length === 1 && (
                  <>
                    <div className="space-y-1 border-t border-slate-700 pt-3 relative z-[70]">
                      <span className="text-[9px] font-bold text-slate-400">Hücre Metni</span>
                      <textarea rows={2} value={refCell!.text} onChange={(e) => updateBannerCell(refCell!.id, { text: e.target.value })} className="w-full text-[10px] font-bold text-slate-200 bg-slate-800 border border-slate-600 rounded p-1.5 outline-none focus:border-blue-500 resize-none" placeholder="Hücreye yazılacak metin..." />
                    </div>
                    <div className="space-y-1 border-t border-slate-700 pt-3 relative z-[60]">
                      <span className="text-[9px] font-bold text-slate-400">Resim Ekle</span>
                      <input type="file" accept="image/*" className="hidden" id="banner-image-upload" ref={fileInputRef} onChange={handleImageUpload} />
                      <div className="flex gap-1">
                        <label htmlFor="banner-image-upload" className="flex-1 h-[30px] flex items-center justify-center text-[10px] font-bold text-slate-200 bg-slate-800 border border-slate-600 hover:border-blue-400 rounded cursor-pointer transition-all">
                          {isUploading ? "Yükleniyor..." : (refCell!.image ? "Resmi Değiştir" : "Bilgisayardan Seç")}
                        </label>
                        {refCell!.image && <button onClick={() => updateBannerCell(refCell!.id, { image: null })} className="h-[30px] px-3 bg-red-50 text-red-600 border border-red-200 rounded text-[10px] font-bold hover:bg-red-100 transition-colors">Sil</button>}
                      </div>
                    </div>
                  </>
                )}
                ...
                <div className="flex items-center justify-between border-t border-slate-700 pt-3 relative z-[50]">
                  <span className="text-[10px] font-bold text-slate-300">Hücre Dolgu Rengi</span>
                  <ColorOpacityPicker color={refCell!.bgColor.c} opacity={refCell!.bgColor.o} onChange={(c, o) => updateSelectedBannerCells({ bgColor: { c, o } })} />
                </div>

                <div className="border-t border-slate-700 pt-3 space-y-2 relative z-[40]">
                  <span className="text-[10px] font-bold text-slate-300">Hücre Çerçevesi (Border)</span>
                  <div className="flex items-center justify-between">
                    ...
                    <select value={refCell!.border.style} onChange={(e) => updateSelectedBannerCells({ border: { style: e.target.value as any } })} className="text-[10px] text-slate-200 bg-slate-800 border border-slate-600 rounded p-1 outline-none">
                  ...
                  <div className="grid grid-cols-5 gap-1 bg-slate-800 p-1.5 rounded border border-slate-700">
                    <button onClick={() => handleBorderChange('linked', refCell!.border.linked ? 0 : 1)} className={`flex items-center justify-center p-1 rounded text-[10px] ${refCell!.border.linked ? 'bg-blue-100 text-blue-600' : 'bg-slate-200 text-slate-500'}`} title={refCell!.border.linked ? "Kenarlar Bağlı" : "Kenarlar Bağımsız"}>
                      {refCell!.border.linked ? "🔗" : "🔓"}
                    </button>
                    {['t', 'r', 'b', 'l'].map(side => (
                      <div key={side} className="flex flex-col items-center relative z-[30]">
                        <span className="text-[8px] text-slate-400 uppercase">{side === 't' ? 'Üst' : side === 'b' ? 'Alt' : side === 'r' ? 'Sağ' : 'Sol'}</span>
                        <input type="number" min="0" max="20" value={refCell!.border[side as 't' | 'r' | 'b' | 'l']} onChange={(e) => { const val = parseInt(e.target.value) || 0; if (refCell!.border.linked) { handleBorderChange('all', val); } else { handleBorderChange(side, val); } }} className="w-full text-[10px] text-center bg-slate-900 text-slate-200 border border-slate-600 rounded p-0.5 outline-none focus:border-blue-500" />
                      </div>
                    ))}
                  </div>
                </div>

                <div className="border-t border-slate-700 pt-3 relative z-[20]">
                ...
                <div className="border-t border-slate-700 pt-3 relative z-[5]">
                  <button 
                    onClick={() => {
