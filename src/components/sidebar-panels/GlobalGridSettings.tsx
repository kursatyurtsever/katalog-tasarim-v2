  ...
  return (
    <div className="space-y-4">
      <div className="bg-white p-3 rounded-md border border-slate-200 shadow-sm space-y-3">
        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Varsayılan Ayarlar</h4>
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input 
              type="number" 
              min="1" max="10" 
              value={globalSettings.defaultGrid.rows}
              onChange={(e) => updateGridSettings("global", { ...globalSettings.defaultGrid, rows: parseInt(e.target.value) || 1 })}
          ...
          <div>
            ...
            <input 
              type="number" 
              min="1" max="10"
              value={globalSettings.defaultGrid.cols}
              onChange={(e) => updateGridSettings("global", { ...globalSettings.defaultGrid, cols: parseInt(e.target.value) || 1 })}
              className="w-full mt-1 text-sm font-bold text-slate-700 border border-slate-200 rounded p-1.5 outline-none focus:border-blue-500"
            />
          </div>
        </div>
      ...
      <div className="bg-white p-3 rounded-md border border-slate-200 shadow-sm space-y-3">
        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Sayfa Bazlı Özelleştirme</h4>
          ...
          {activeForma?.pages.map(page => {
            ...
            return (
              <button
                ...
                className={`relative min-w-[36px] h-9 rounded border text-xs font-black transition-all ${
