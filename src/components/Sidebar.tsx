...
export function Sidebar() {
  ...
  // Akordeon menü için state
  const [openAcc, setOpenAcc] = useState<string | null>("price");
      ...
      <div className="flex-1 overflow-y-auto flex flex-col">
        ...
        {activeTab === "settings" && (
          <div className="p-4 bg-slate-50/50 h-full overflow-y-auto custom-scrollbar">
            ...
            {/* 2. FİYAT AYARLARI AKORDEONU */}
            <div className="bg-white rounded-md border border-slate-200 shadow-sm overflow-hidden mb-20">
              <button 
                onClick={() => setOpenAcc(openAcc === "price" ? null : "price")} 
                className="w-full flex items-center justify-between p-3 bg-slate-800 hover:bg-slate-700 transition-colors"
              >
                <span className="text-[11px] font-black text-white uppercase tracking-widest">Fiyat Ayarları</span>
                <span className="text-white text-xs">{openAcc === "price" ? "▲" : "▼"}</span>
              </button>
              
              {openAcc === "price" && (
                <div className="p-4 bg-slate-50 border-t border-slate-200 space-y-6">
                  
                  {/* --- FIYAT KENAR AYARLARI --- */}
                  <div className="space-y-3">
                    ...
                    {globalSettings.linkPriceRadius ? (
                      <div className="flex items-center justify-between gap-2 bg-white p-2 rounded border border-slate-200">
                        <span className="text-[10px] font-medium text-slate-500 w-16">Yarıçap</span>
                        <input type="range" min="0" max="30" value={globalSettings.priceRadiusTL} onChange={(e) => { const v = parseInt(e.target.value); setGlobalSettings({ priceRadiusTL: v, priceRadiusTR: v, priceRadiusBR: v, priceRadiusBL: v }); }} className="flex-1 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                      ...
                      <div className="grid grid-cols-2 gap-x-3 gap-y-2 bg-white p-2 rounded border border-slate-200">
                        {['TL', 'TR', 'BL', 'BR'].map((pos) => (
                          <div key={pos} className="flex items-center justify-between gap-1">
                            <span className="text-[9px] font-medium text-slate-400 w-4">{pos}</span>
                            <input type="range" min="0" max="30" value={(globalSettings as any)[`priceRadius${pos}`]} onChange={(e) => setGlobalSettings({ [`priceRadius${pos}`]: parseInt(e.target.value) })} className="flex-1 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                            <span className="text-[9px] font-bold text-slate-600 w-4 text-right">{Math.floor((globalSettings as any)[`priceRadius${pos}`])}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-3 bg-white p-2 rounded border border-slate-200">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <input type="range" min="10" max="40" value={globalSettings.priceFontSize} onChange={(e) => setGlobalSettings({ priceFontSize: parseInt(e.target.value) })} className="flex-1 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <input type="range" min="8" max="30" value={globalSettings.priceDecimalSize} onChange={(e) => setGlobalSettings({ priceDecimalSize: parseInt(e.target.value) })} className="flex-1 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                        <span className="text-[10px] font-black text-orange-500 w-6 text-right">{globalSettings.priceDecimalSize}</span>
                      </div>
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[10px] font-medium text-slate-500 w-20">Harf Aralığı (VA)</span>
                        <input type="range" min="-5" max="10" step="0.5" value={globalSettings.priceLetterSpacing} onChange={(e) => setGlobalSettings({ priceLetterSpacing: parseFloat(e.target.value) })} className="flex-1 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="bg-white p-2 rounded border border-slate-200 space-y-2 flex flex-col justify-between">
                        ...
                        <div className="flex flex-col bg-slate-50 rounded p-1 gap-1 border border-slate-100 flex-1">
                          {['left', 'center', 'right', 'justify'].map((align) => (
                            <button key={align} onClick={() => setGlobalSettings({ priceTextAlign: align as any })} className={`w-full py-1 text-[9px] font-bold rounded transition-all ${globalSettings.priceTextAlign === align ? 'bg-white shadow border border-slate-200 text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
                              {align === 'left' ? 'Sol' : align === 'center' ? 'Orta' : align === 'right' ? 'Sağ' : 'Yasla'}
                            </button>
                          ))}
                        </div>
                      ...
                      <div className="bg-white p-2 rounded border border-slate-200 space-y-2 flex flex-col justify-between">
                        ...
                        <div className="flex flex-col bg-slate-50 rounded p-1 gap-1 border border-slate-100 flex-1">
                          {['top', 'middle', 'bottom'].map((val) => (
                            <button key={val} onClick={() => setGlobalSettings({ priceTextVerticalAlign: val as any })} className={`w-full py-1 text-[9px] font-bold rounded transition-all ${globalSettings.priceTextVerticalAlign === val ? 'bg-white shadow border border-slate-200 text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
                              {val === 'top' ? 'Üst' : val === 'middle' ? 'Orta' : 'Alt'}
                            </button>
                          ))}
                        </div>
